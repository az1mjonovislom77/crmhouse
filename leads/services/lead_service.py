...
class LeadService:
    ...
    def update_lead(instance, data, user):
        ...
        if new_status and new_status != instance.status:
            ...
            LeadEvent.objects.create(
                lead=instance,
                type=LeadEvent.TYPE_SUB_STATUS,
                from_value=instance.sub_status,
                to_value=new_sub,
                by=user,
            )
            instance.sub_status = new_sub
        ...
        if new_subsidiya is not None and new_subsidiya != instance.subsidiya:
            LeadEvent.objects.create(
                lead=instance,
                type=LeadEvent.TYPE_SUBSIDIYA,
                from_value=str(instance.subsidiya),
                to_value=str(new_subsidiya),
                subsidiya=new_subsidiya,
                by=user,
            )
        ...
        if meeting_at and meeting_type:
            LeadEvent.objects.create(
                lead=instance, type=LeadEvent.TYPE_MEETING, meeting_at=meeting_at, meeting_type=meeting_type, by=user
            )
