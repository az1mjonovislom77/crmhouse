    ...
    def handle(self, *args, **options):
        ...
        for row_num, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            try:
                ...
                status, sub_status = STATUS_MAP.get(status_key, ("yangi_murojaat", "murojaat_qildi"))
                ...
                if phone in existing_phones:
                    if contact_dt:
                        Lead.objects.filter(phone=phone, contacted_at__isnull=True).update(contacted_at=contact_dt)
                    duplicate_count += 1
                    continue
