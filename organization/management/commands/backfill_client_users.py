...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for client_id, user_id in rows:
            if user_id:
                updated += Client.objects.filter(pk=client_id, user__isnull=True).update(user_id=user_id)

        orgs = Client.objects.filter(organization__isnull=True, user__organization__isnull=False).values_list(
        ...
        self.stdout.write(self.style.SUCCESS(f"Client: {updated} ta user, {org_updated} ta organization yangilandi"))
