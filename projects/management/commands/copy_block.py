...
class Command(BaseCommand):
    ...
    def copy_image(self, name):
        ...
        if not default_storage.exists(name):
            self.stdout.write(self.style.WARNING(f"  Fayl storage'da topilmadi, eski yo'l bilan ulanadi: {name}"))
            return name
    ...
    def handle(self, *args, **options):
        ...
        if Block.objects.filter(title=new_title, projects=src_block.projects).exists():
            self.stdout.write(
                self.style.ERROR(f'"{new_title}" nomli block shu projectda allaqachon mavjud, avval uni tekshiring')
            )
            return
        ...
        self.stdout.write(f'"{src_block.title}" (id={src_block.pk}, project="{src_block.projects}") -> "{new_title}"')
        self.stdout.write(
        ...
        with transaction.atomic():
            ...
            BlockImage.objects.bulk_create(
                [BlockImage(block=new_block, image=self.copy_image(bi.image.name)) for bi in block_images]
            )
