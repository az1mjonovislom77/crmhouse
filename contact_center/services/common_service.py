...
def build_user_phone_map() -> dict[str, int]:
    ...
    for user_id, phone in (
        User.objects.exclude(phone_number__isnull=True).exclude(phone_number="").values_list("id", "phone_number")
    ):
...
def match_user_id(phone_map: dict[str, int], *numbers: str | None) -> int | None:
...
class ExternalAPIService:
    ...
    def fetch_cdr_data(cls, params: dict):
        ...
        response = requests.get(
            url, headers=headers, params=params, timeout=cls.TIMEOUT, verify=settings.PBX_VERIFY_SSL
        )
        if response.status_code != 200:
        ...
        try:
            ...
            raise APIException("CDR olishda xato: javob JSON emas") from err
