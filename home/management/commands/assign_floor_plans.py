...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--count", type=int, default=0, help="Nechta rasm ishlatish (default: 0 = barchasi). Masalan: --count 4"
        )
    ...
    def handle(self, *args, **kwargs):
        ...
        image_files = sorted(
            [
                ...
                if f.lower().endswith(ALLOWED_EXT) and (not prefix or f.startswith(prefix))
            ],
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"\nNatija: {len(to_create)} ta floor plan yaratildi ({cycle_size} ta rasm cycling tartibida)"
            )
        )
