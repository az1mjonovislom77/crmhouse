...
def get_sold_events(date_from=None, date_to=None):
    reverted = HomeStatusHistory.objects.filter(
        home=OuterRef("home"), changed_at__gt=OuterRef("changed_at"), from_status__in=SOLD_STATUSES
    ).exclude(to_status__in=[*SOLD_STATUSES, "booking_deleted"])
    qs = (
        HomeStatusHistory.objects.filter(to_status__in=SOLD_STATUSES)
...
def get_top_sellers(qs, limit=5):
    ...
    for row in rows:
        seller = sellers.setdefault(
            row["changed_by"],
            {"name": row["changed_by__full_name"], "sold": 0, "revenue": Decimal(0)},
        )
        seller["sold"] += 1
...
def get_block_occupancy(qs):
    rows = (
        ...
        .annotate(total=Count("id"), occupied=Count("id", filter=~Q(home_status=Home.HomeStatus.AVAILABLE)))
